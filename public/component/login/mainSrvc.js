angular.module("servproj").service("mainSrvc", function($http, $state) {
  ...
  this.deletePrepMeal = current => {
    // from mainCtrl IwantoDeleteMeal and currentmeal.id (nickname of CORE name: currentmeal.id)
  ...
  this.addToCart = (fname, fschedule, fmealcost, fid, fauthid) => {
    ...
    return $http
      .post("/api/addToCart/", { fname, fschedule, fmealcost, fid, fauthid })
      .then(function() {
        alert(fname + " is now in your cart");
      });
  };
